    ...
    <div className="flex flex-col gap-8">
      <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <h1 className="text-4xl font-bold tracking-tight">Chats com Memória Persistente</h1>
      ...
      <div className="grid min-h-[calc(100vh-12rem)] grid-cols-1 gap-8 xl:grid-cols-[320px_minmax(0,1fr)_280px]">
          ...
          <div className="max-h-[72vh] space-y-2 overflow-y-auto p-3">
            {isLoadingSessions ? (
              Array.from({ length: 5 }).map((_, index) => (
                <div key={`session-loading-${index}`} className="h-24 animate-pulse rounded-[1.4rem] bg-slateblue/5" />
              ))
            ) : (
              sessions.map((session) => (
                ...
                >
                  ...
                  >
                    ...
                    <p className="mt-2 line-clamp-2 text-xs leading-relaxed text-slateblue/75">
                      {session.last_message_preview || "Chat pronto para iniciar."}
                    </p>
                    <p className="mt-3 text-[0.65rem] font-bold uppercase tracking-[0.2em] text-slateblue/45">
        ...
        <GlassCard className="flex flex-col overflow-hidden !p-0">
          <div className="border-b border-white/50 px-6 py-5">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-bold">{activeSession?.title || "Novo chat"}</h2>
              </div>
              <div className="flex flex-wrap gap-2">
                <span className="chat-session-meta">{messages.length} mensagens</span>
                <span className="chat-session-meta">{references.length} referências</span>
              </div>
            </div>
          </div>
            ...
            {isLoadingConversation ? (
              <div className="space-y-4">
                {Array.from({ length: 4 }).map((_, index) => (
                  <div key={`message-loading-${index}`} className="h-20 animate-pulse rounded-[1.6rem] bg-slateblue/5" />
                ))}
              </div>
            ) : messages.length === 0 ? (
              <div className="flex h-full flex-col items-center justify-center px-10 text-center opacity-50">
                <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-slateblue/10">
                  <ChatOrbIcon />
                </div>
                <p className="text-xl font-bold italic text-slateblue">Este chat ainda está vazio.</p>
                <p className="mt-2 max-w-md text-sm text-slateblue/80">
                  ...
                  <div key={`${message.timestamp}-${index}`} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                    <div className={`chat-bubble shadow-sm ${message.role === "user" ? "chat-user" : "chat-assistant"}`}>
                      <div className="mb-2 flex items-center justify-between gap-4">
                        <p className="text-[0.68rem] font-bold uppercase tracking-[0.18em] opacity-55">
                          {message.role === "user" ? "Operador" : "Nexus IA"}
                        </p>
                        <p className="text-[0.68rem] font-semibold opacity-45">
                  ...
                  <div className="flex justify-start">
                    <div className="chat-assistant chat-bubble shadow-sm animate-pulse">
                      <div className="flex gap-1">
                        <div className="h-1.5 w-1.5 animate-bounce rounded-full bg-slateblue/40" />
                        <div className="h-1.5 w-1.5 animate-bounce rounded-full bg-slateblue/40 [animation-delay:0.2s]" />
                        <div className="h-1.5 w-1.5 animate-bounce rounded-full bg-slateblue/40 [animation-delay:0.4s]" />
                      </div>
                    </div>
                  </div>
                )}
          ...
          <form onSubmit={handleChat} className="border-t border-white/60 bg-white/40 p-6">
            <div className="flex gap-3">
              <Input
                value={chatInput}
                onChange={(event) => setChatInput(event.target.value)}
                placeholder="Digite sua pergunta para este chat..."
        ...
        <GlassCard className="!p-0 overflow-hidden">
          <div className="border-b border-white/50 px-6 py-5">
            ...
            <p className="mt-2 text-sm text-slateblue">
              Referências usadas na última resposta desta conversa.
            </p>
          </div>

          <div className="max-h-[72vh] space-y-4 overflow-y-auto p-4">
            {references.length === 0 ? (
              <p className="px-2 text-[0.75rem] italic text-slateblue/55">
                As referências do chat selecionado aparecerão aqui quando a IA utilizar documentos do seu acervo.
              </p>
            ) : (
              references.map((reference, index) => (
                <div key={`${reference.document_id || "ref"}-${index}`} className="rounded-2xl border border-white/40 bg-white/60 p-4 text-[0.76rem]">
                  ...
                  {reference.document_id && (
                    <p className="mt-3 text-[0.65rem] font-bold uppercase tracking-[0.18em] text-amber-700">
