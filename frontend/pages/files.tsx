    ...
    <div className="space-y-8">
      <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <h1 className="text-4xl font-bold tracking-tight">Gerenciador de Arquivos</h1>
          <p className="mt-2 max-w-3xl text-slateblue">
            Navegue pelas pastas do seu acervo privado, visualize os arquivos organizados pela IA e faça download do PDF original quando precisar.
          </p>
        </div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <MetricCard label="Arquivos" value={String(documents.length)} help="PDFs disponíveis no seu espaço privado" />
          <MetricCard label="Pastas" value={String(Math.max(folderMap.size - 1, 0))} help="Estrutura derivada do seu manifesto" />
          <MetricCard label="Volume" value={formatBytes(totalSize)} help="Estimativa total do acervo salvo" />
        </div>
      </header>

      <GlassCard className="!p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-wrap items-center gap-2 text-sm font-semibold text-slateblue">
            <button
              type="button"
              onClick={() => setCurrentPath("")}
              className={`rounded-full px-3 py-1 transition-colors ${currentPath === "" ? "bg-ink text-white" : "bg-white/70 hover:bg-white"}`}
            >
              Minha unidade
            </button>
            {breadcrumbs.map((crumb) => (
              <div key={crumb.path} className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={`rounded-full px-3 py-1 transition-colors ${crumb.path === currentPath ? "bg-ink text-white" : "bg-white/70 hover:bg-white"}`}
                >
                  {crumb.label}
                </button>
              </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <input
              ...
              className="field min-w-[260px] !rounded-full !py-3"
              placeholder="Filtrar pasta, título, arquivo, tag..."
            />
      ...
      <div className="grid grid-cols-1 gap-8 xl:grid-cols-[300px_minmax(0,1fr)]">
        <GlassCard className="!p-0 overflow-hidden">
          <div className="border-b border-white/50 px-6 py-5">
            <p className="eyebrow">Pastas</p>
            <p className="mt-2 text-sm text-slateblue">Estrutura privada do usuário autenticado.</p>
          </div>
        ...
        <div className="space-y-6">
          <GlassCard className="overflow-hidden !p-0">
            ...
            <div className="grid grid-cols-1 gap-4 border-b border-white/50 p-6 md:grid-cols-2 xl:grid-cols-3">
              {folderChildren.length === 0 ? (
                <div className="rounded-[1.6rem] border border-dashed border-slateblue/20 bg-white/40 p-5 text-sm text-slateblue/60">
                ...
                folderChildren.map((folder) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className="truncate text-sm font-bold text-ink">{folder.name}</p>
                      <p className="mt-1 text-[0.68rem] font-bold uppercase tracking-[0.18em] text-slateblue/55">
                  ...
                  {loading ? (
                    Array.from({ length: 4 }).map((_, index) => (
                      <tr key={`skeleton-${index}`}>
                        <td colSpan={6}>
                          <div className="h-10 animate-pulse rounded-2xl bg-slateblue/5" />
                        </td>
                      </tr>
                    ))
                  ) : visibleFiles.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="py-16 text-center text-slateblue/50">
                    ...
                    visibleFiles.map((document) => (
                      <tr key={document.document_id}>
                        <td>
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="truncate text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-slateblue/55">
                        ...
                        <td className="max-w-[240px]">
                          <p className="truncate font-semibold text-ink/80">{document.title}</p>
                          <p className="mt-1 truncate text-xs text-slateblue/60">{document.summary || "Sem resumo disponível."}</p>
                        </td>
                        <td>
                          <StatusChip label={document.classification || "arquivo"} variant="info" />
                        </td>
                        <td className="text-xs">{formatDate(document.uploaded_at)}</td>
                        <td className="text-xs font-semibold text-slateblue">{formatBytes(getDocumentSize(document))}</td>
                        <td>
                          <Button
                            type="button"
                            variant="secondary"
...
function FolderTree({
  ...
  return (
    <div className={path ? "ml-4 border-l border-white/40 pl-4" : ""}>
      <button
        type="button"
        onClick={() => onSelect(path)}
        className={`folder-tree-item ${currentPath === path ? "folder-tree-item-active" : ""}`}
      >
        <FolderIcon />
